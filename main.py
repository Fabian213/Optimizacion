...
if __name__ == '__main__':
    ...
    if not os.path.exists('soluciones'):
        os.mkdir('soluciones')
    if args.benchmark:
        for file in glob.glob('Intancias/*.txt'):
            problem = SolomonFormatParser(file).get_problem()
            solution = IteratedLocalSearch(problem).execute()
            with open(f"""soluciones/{file.split(os.sep)[1].split(".")[0]}.sol""", 'w') as f:
        ...
        print(problem)
        solution = IteratedLocalSearch(problem).execute()
        with open(f"""soluciones/{args.problem_file.split(os.sep)[-1].split(".")[0]}.sol""", 'w') as f:
            f.write(problem.print_canonical(solution))
